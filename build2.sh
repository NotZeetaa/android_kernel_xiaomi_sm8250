...
chatid=-542983190
token=1787514307:AAEolLZxzYN9ZJbzaiWMDXVEgAsfkUkbdl8
...
ZIPNAME=Nexus
FINAL_ZIP=${ZIPNAME}-${DEVICE}-KERNEL-AOSP-${TANGGAL}.zip
FINAL_ZIP2=${ZIPNAME}-${DEVICE}-KERNEL-MIUI-${TANGGAL}.zip
...
function cloneTC() {
	...
	post_msg " Cloning Aosp Clang 14.0.1 ToolChain "
		mkdir aosp-clang
        cd aosp-clang || exit
	wget -q https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/refs/heads/master/clang-r450784.tar.gz
        tar -xf clang*
        cd .. || exit
